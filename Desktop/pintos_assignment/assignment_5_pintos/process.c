...
static void process_free_children(struct thread* t);
...
/*
MODIFIED IN PINTOS ASSIGNMENT 4, 5.
...
process_execute(const char* command)
{
  ...
  if (tid == TID_ERROR) {
    ...
    /* ADDITIONS AND CHANGES IN PINTOS ASSIGNMENT 4 and 5. */
    struct thread* child = thread_find(tid);
    child->parent_thread = cur;
    ASSERT(child != NULL);

    sema_down(&child->sema_child_loaded);

    if (cur->child_loaded == true) {

      // Set the parent of the just created thread.
      struct thread* child = thread_find(tid);
      child->parent_thread = cur;

      struct child_data* d = malloc(sizeof(struct child_data));

      d->tid = child->tid;
      d->exit_status = ERROR_EXIT_STATUS;
      list_push_back(&cur->children, &d->elem);
...
start_process(void* command)
{
  ...
  if (cur->parent_thread) {
    cur->parent_thread->child_loaded = success;
  }
  sema_up(&cur->sema_child_loaded);
...
process_wait(tid_t child_tid)
{
  ...
  */
  ...
  struct child_data* cd = child_data_find(child_tid, cur);
  ASSERT(cd != NULL);
  list_remove(&cd->elem);

  int exit_status = cd->exit_status;
  free(cd);
...
static void
process_free_children(struct thread* t)
{
  struct list_elem* e;
...
process_exit()
{
  ...
  process_free_children(cur);

  /* ADDED IN PINTOS ASSIGNMENT 4. */
  if (cur->parent_waiting) {
    sema_up(&cur->parent_thread->sema_child_wait);
    cur->parent_waiting = !cur->parent_waiting;
  }
}
